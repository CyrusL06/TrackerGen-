...
export function AuthField({
  ...
  const input = (
    <input
      ...
      required={required}
      disabled={disabled}
      className={`min-h-[3rem] rounded-[12px] border border-[color:var(--auth-border)] bg-[rgba(255,255,255,0.04)] px-3.5 text-body-sm text-[color:var(--auth-text)] outline-none transition-colors placeholder:text-[color:var(--auth-muted)] focus:border-[color:var(--auth-border-strong)] focus-visible:ring-1 focus-visible:ring-[rgba(255,255,255,0.15)] motion-reduce:transition-none disabled:cursor-not-allowed disabled:opacity-50 ${prefix ? "pl-10" : ""} ${FONTS.body} ${inputClassName}`}
    />
  );
  ...
  return (
    <label className={`grid gap-2 ${className}`} style={vars}>
      <div className="flex items-center justify-between gap-4">
        <span
          className={`text-caption tracking-[0.02em] text-[color:var(--auth-muted)] ${FONTS.body}`}
        >
          {label}
        </span>
        {auxiliary}
      ...
      {prefix ? (
        <div className="relative">
          <span
            className={`pointer-events-none absolute left-3.5 top-1/2 -translate-y-1/2 text-body-sm text-[color:var(--auth-muted)] ${FONTS.body}`}
          >
            {prefix}
          </span>
          {input}
      ...
      {helpText ? (
        <span className={`text-tiny leading-5 text-[color:var(--auth-muted)] ${FONTS.body}`}>
          {helpText}
        </span>
      ) : null}
    </label>
...
export function AuthDivider() {
  return (
    <div className="flex items-center gap-4" style={vars}>
      ...
      <span className={`text-caption text-[color:var(--auth-muted)] ${FONTS.body}`}>or</span>
      <div className="h-px flex-1 bg-[color:var(--auth-border)]" />
    </div>
  );
}
...
export function AuthPrimaryButton({
  ...
  return (
    <button
      ...
      disabled={disabled}
      style={vars}
      className={`inline-flex min-h-[3rem] items-center justify-center rounded-[12px] border border-[color:var(--auth-border)] bg-[rgba(255,255,255,0.04)] px-4 text-body-sm transition-colors focus-visible:border-[color:var(--auth-border-strong)] focus-visible:ring-1 focus-visible:ring-[rgba(255,255,255,0.15)] motion-reduce:transition-none ${
        disabled
...
export function AuthAltLink({ prompt, cta, to }) {
  return (
    <p className={`text-center text-body-sm leading-5 text-[color:var(--auth-muted)] ${FONTS.body}`} style={vars}>
      {prompt}{" "}
...
export function AuthFinePrint({ children }) {
  return (
    <p className={`mx-auto max-w-[22rem] text-center text-tiny leading-5 text-[color:var(--auth-muted)] ${FONTS.body}`} style={vars}>
      {children}
    </p>
  );
}
